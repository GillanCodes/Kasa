...
export default function Slideshow({ pictures }) { //Pass an array of pics
    ...
    return (
        <div className="slideshow">
            {/* Prevuis btn */}
            {pictures.length > 1 ? <span id='prev' onClick={previus}><i className="fa-solid fa-angle-left"></i></span>: ""}
            ...
            {/* Next btn */}
            {pictures.length > 1 ? <span id='next' onClick={next}><i className="fa-solid fa-chevron-right"></i></span>: ""}
            {/* Little text to show user where he is on how mush pics the rent has */}
            {pictures.length > 1 ? <p id='indicator'>{slide + 1}/{pictures.length}</p> : ""}
        </div>
    )
}
